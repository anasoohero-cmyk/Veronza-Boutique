...
const corsHeaders={'Access-Control-Allow-Origin':'*','Access-Control-Allow-Methods':'GET,PATCH,OPTIONS','Access-Control-Allow-Headers':'Content-Type,Authorization'};
...
module.exports=async(req,res)=>{
 ...
 if(!SUPABASE_URL||!SERVICE_KEY)return json(res,500,{error:'Server configuration is incomplete'});const user=await requireAdmin(req);if(!user)return json(res,401,{error:'Unauthorized'});
 if(req.method==='GET'){const q=new URL(req.url,'http://veronza.local').searchParams;const id=q.get('order_id');if(id){const r=await sbFetch(`/rest/v1/order_items?select=id,product_code,product_name,color,size,quantity,unit_price,image_url,created_at&order_id=eq.${encodeURIComponent(id)}&order=created_at.asc`);if(!r.ok)return json(res,502,{error:'Could not load order items'});return json(res,200,{items:r.data})}const r=await sbFetch('/rest/v1/orders?select=id,order_number,user_id,customer_name,customer_phone,customer_address,total,status,admin_notes,created_at,updated_at,whatsapp_status,whatsapp_last_error,whatsapp_sent_at&order=created_at.desc');if(!r.ok)return json(res,502,{error:'Could not load orders'});return json(res,200,{orders:r.data})}
 if(req.method==='PATCH'){let body=req.body||{};if(typeof body==='string'){try{body=JSON.parse(body)}catch{return json(res,400,{error:'Invalid JSON'})}}const allowed=['pending','confirmed','preparing','shipped','delivered','cancelled','returned'];if(!body.id||!allowed.includes(body.status))return json(res,400,{error:'Invalid order or status'});
  ...
  return json(res,200,{order:updatedOrder,notify})}
