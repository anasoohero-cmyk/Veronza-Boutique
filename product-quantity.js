(()=>{
  ...
  async function loadSupabaseClient(){
    ...
    supabaseClientPromise=new Promise((resolve,reject)=>{
      ...
      timer=setTimeout(()=>finish(reject,new Error('انتهت مهلة تحميل Supabase')),PRODUCT_LOAD_TIMEOUT);
  ...
  async function syncQuantity(){
    try{
      ...
      const {data,error}=await client.from('products').select('id,quantity').eq('is_active',true).order('id',{ascending:true});
      if(error)throw error;
      const quantities=new Map((Array.isArray(data)?data:[]).map(row=>[Number(row.id),Number(row.quantity||0)]));
      if(Array.isArray(window.products))window.products.forEach(p=>{if(quantities.has(Number(p.id)))p.quantity=quantities.get(Number(p.id))});
