...
module.exports=async(req,res)=>{
  ...
  const token=auth.replace(/^Bearer\s+/i,'').trim(); let user; try{const r=await fetch(`${supabaseUrl}/auth/v1/user`,{headers:{apikey:process.env.SUPABASE_ANON_KEY||process.env.SUPABASE_PUBLISHABLE_KEY||serviceKey,Authorization:`Bearer ${token}`}});if(!r.ok)return res.status(401).json({ok:false,error:'Invalid session'});user=await r.json()}catch(_){return res.status(401).json({ok:false,error:'Invalid session'})}
  const h={apikey:serviceKey,Authorization:`Bearer ${serviceKey}`,'Content-Type':'application/json'};
  const a=await fetch(`${supabaseUrl}/rest/v1/admin_users?select=user_id&user_id=eq.${encodeURIComponent(user.id)}&limit=1`,{headers:h}); if(!a.ok||!(await a.json()).length)return res.status(403).json({ok:false,error:'Admin access required'});
  const c=await fetch(`${supabaseUrl}/rest/v1/push_config?select=vapid_public_key&id=eq.true&limit=1`,{headers:h}); if(!c.ok)return res.status(500).json({ok:false,error:'Push configuration unavailable'}); const row=(await c.json())[0]; if(!row)return res.status(500).json({ok:false,error:'Push configuration unavailable'}); return res.status(200).json({ok:true,publicKey:row.vapid_public_key});
};
