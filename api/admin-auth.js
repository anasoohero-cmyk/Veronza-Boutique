...
module.exports = async (req, res) => {
  ...
  if (!SUPABASE_URL || !SERVICE_KEY)
    return json(req, res, 500, { error: 'Server configuration incomplete' });
  ...
  try {
    const uResp = await fetch(`${SUPABASE_URL}/auth/v1/user`, {
      headers: {
        apikey:
          process.env.SUPABASE_ANON_KEY || process.env.SUPABASE_PUBLISHABLE_KEY || SERVICE_KEY,
    ...
    let aData;
    try {
      aData = aText ? JSON.parse(aText) : null;
    } catch {
      aData = aText;
    }
    if (!aResp.ok || !Array.isArray(aData) || !aData.length)
    ...
    return json(req, res, 401, {
      ...
      debug_message: String((e && e.message) || e),
    });
  }
};
