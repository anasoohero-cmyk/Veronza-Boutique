module.exports = async (req, res) => {
  ...
  if (whatsappError) { const r = await fetch(`${supabaseUrl}/rest/v1/orders?id=eq.${encodeURIComponent(orderId)}`, { method: 'PATCH', headers: { ...headers, Prefer: 'return=minimal' }, body: JSON.stringify({ whatsapp_status: 'failed', whatsapp_last_error: whatsappError }) }); if (!r.ok) console.error('Failed to record WhatsApp delivery failure', await r.text()); return res.status(502).json({ ok: false, error: 'Order was reserved, but WhatsApp delivery failed', order_id: orderId, order_number: orderNumber, whatsapp_status: 'failed' }); }
  const sent = await fetch(`${supabaseUrl}/rest/v1/orders?id=eq.${encodeURIComponent(orderId)}`, { method: 'PATCH', headers: { ...headers, Prefer: 'return=minimal' }, body: JSON.stringify({ whatsapp_status: 'sent', whatsapp_last_error: null, whatsapp_sent_at: new Date().toISOString() }) });
  if (!sent.ok) console.error('Failed to record WhatsApp sent status', await sent.text());
