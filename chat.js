...
module.exports = async (req, res) => {
  ...
  if (!supabaseUrl || !serviceKey) return json(res, 500, { ok: false, error: 'Server configuration incomplete' });
  ...
  if (req.method === 'GET') {
    ...
    if (!isUuid(conversationId) || !isUuid(guestToken)) return json(res, 400, { ok: false, error: 'Invalid identifiers' });
    ...
    if (!conversation || conversation.guest_token !== guestToken) return json(res, 404, { ok: false, error: 'Conversation not found' });
    ...
    if (!msgResp.ok) return json(res, 502, { ok: false, error: 'Could not load messages' });
    ...
    return json(res, 200, { ok: true, status: conversation.status, messages: msgResp.data || [] });
  }
  ...
  if (action === 'start') {
    ...
    if (!insertResp.ok) return json(res, 502, { ok: false, error: 'Could not start conversation' });
    const conversation = Array.isArray(insertResp.data) ? insertResp.data[0] : insertResp.data;
    return json(res, 200, { ok: true, conversation_id: conversation.id, guest_token: guestToken });
  }
  ...
  if (action === 'send') {
    ...
    if (!isUuid(conversationId) || !isUuid(guestToken)) return json(res, 400, { ok: false, error: 'Invalid identifiers' });
    if (!text) return json(res, 400, { ok: false, error: 'Message body required' });
    ...
    if (!insertMsg.ok) return json(res, 502, { ok: false, error: 'Could not send message' });
