(()=>{
  ...
  const fmtTime=d=>new Date(d).toLocaleString('ar-LY',{dateStyle:'short',timeStyle:'short'});
  const init=async(client)=>{
    const {data:{session}}=await client.auth.getSession(); if(!session)return;
    const {data:admin}=await client.from('admin_users').select('user_id').eq('user_id',session.user.id).maybeSingle(); if(!admin)return;

    if(!document.getElementById('vz-chatw-style')){
      const s=document.createElement('style');
      s.id='vz-chatw-style';
      s.textContent=`
        #vz-chatw-btn{position:relative}
        .vz-chatw-badge{position:absolute;top:1px;right:1px;background:#b21f2d;color:#fff;border-radius:99px;min-width:17px;height:17px;font:700 10px/17px Arial;text-align:center}
    ...
    const shade=document.createElement('div');shade.className='vz-chatw-shade';document.body.appendChild(shade);
    const panel=document.createElement('aside');panel.className='vz-chatw-panel';
    panel.innerHTML=`
    ...
    let conversations=[],activeId=null,activeMessages=[];

    const initials=name=>{const n=(name||'؟').trim();return n?n[0]:'؟'};

    const renderList=()=>{
      const unreadCount=conversations.filter(c=>c.admin_unread).length;
      badge.hidden=!unreadCount;badge.textContent=unreadCount>99?'99+':unreadCount;
      if(!conversations.length){listEl.innerHTML='<div class="vz-chatw-empty">ما فيه محادثات حتى الآن.</div>';return}
      listEl.innerHTML=conversations.map(c=>`<div class="vz-chatw-row" data-id="${c.id}">${c.admin_unread?'<span class="vz-chatw-unread"></span>':''}<div class="vz-chatw-avatar">${esc(initials(c.customer_name))}</div><div class="vz-chatw-rowinfo"><div class="vz-chatw-name">${esc(c.customer_name||'زائر')}${c.status==='closed'?' · منتهية':''}</div><div class="vz-chatw-preview">${esc(c.last_message_preview||'')}</div></div><div class="vz-chatw-time">${fmtTime(c.last_message_at)}</div></div>`).join('');
      listEl.querySelectorAll('[data-id]').forEach(el=>el.onclick=()=>openThread(el.dataset.id));
    };

    const loadConversations=async()=>{
      const {data,error}=await client.from('chat_conversations').select('id,customer_name,customer_phone,status,admin_unread,last_message_at').order('last_message_at',{ascending:false}).limit(200);
      if(error)return;
      const withPreview=await Promise.all((data||[]).map(async c=>{
        const {data:last}=await client.from('chat_messages').select('body,sender').eq('conversation_id',c.id).order('created_at',{ascending:false}).limit(1).maybeSingle();
        return {...c,last_message_preview:last?(last.sender==='admin'?'أنت: ':'')+last.body:''}
      }));
    ...
    const renderMessages=()=>{
      messagesEl.innerHTML=activeMessages.map(m=>`<div class="vz-chatw-msg ${m.sender}">${esc(m.body)}<time>${fmtTime(m.created_at)}</time></div>`).join('')||'<div class="vz-chatw-empty">ما فيه رسائل حتى الآن.</div>';
    ...
    const openThread=async(id)=>{
      ...
      const {data,error}=await client.from('chat_messages').select('id,sender,body,created_at').eq('conversation_id',id).order('created_at',{ascending:true});
      if(!error){activeMessages=data||[];renderMessages()}
      if(conv&&conv.admin_unread){
        await client.from('chat_conversations').update({admin_unread:false}).eq('id',id);
        conv.admin_unread=false;renderList();
      }
      setTimeout(()=>replyInput.focus(),200);
    };

    const backToList=()=>{activeId=null;panel.classList.remove('thread');titleEl.textContent='المحادثات'};
    ...
    replyForm.addEventListener('submit',async e=>{
      ...
      const {data,error}=await client.from('chat_messages').insert({conversation_id:activeId,sender:'admin',body:text}).select().single();
      if(!error){
        await client.from('chat_conversations').update({customer_unread:true,last_message_at:new Date().toISOString(),status:'open'}).eq('id',activeId);
        activeMessages.push(data);renderMessages();loadConversations();
      }
      replyInput.value='';replyInput.disabled=false;replyInput.focus();
    });
    ...
    client.channel('veronza-admin-chat-widget').on('postgres_changes',{event:'INSERT',schema:'public',table:'chat_messages'},payload=>{
      ...
      loadConversations();
      if(msg.sender==='customer'){try{new Audio('data:audio/wav;base64,UklGRiQAAABXQVZFZm10IBAAAAABAAEAESsAACJWAAACABAAZGF0YQAAAAA=').play()}catch(_){}}
    }).subscribe();
