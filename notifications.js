(()=>{
  ...
  const init=async(client)=>{
    ...
    const btn=document.createElement('button');btn.id='vz-notify';btn.className='icon-btn';btn.type='button';btn.setAttribute('aria-label','الإشعارات');btn.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>';(document.querySelector('.top-actions')||document.querySelector('.header-actions'))?.appendChild(btn); if(!btn.isConnected)return;
