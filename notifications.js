(()=>{
  ...
  const init=async(client)=>{
    ...
    if(!document.getElementById('vz-notify-style')){const s=document.createElement('style');s.id='vz-notify-style';s.textContent=`#vz-notify{position:relative}.vz-badge{position:absolute;top:1px;right:1px;background:#b21f2d;color:#fff;border-radius:99px;min-width:17px;height:17px;font:700 10px/17px Arial;text-align:center}.vz-panel{position:fixed;inset:0 0 0 auto;width:min(94vw,390px);background:#fff;z-index:180;box-shadow:-10px 0 35px #0002;transform:translateX(105%);transition:.25s;display:flex;flex-direction:column}.vz-panel.open{transform:none}.vz-nhead{display:flex;align-items:center;justify-content:space-between;padding:16px 18px;padding-top:max(16px,calc(env(safe-area-inset-top) + 10px));border-bottom:1px solid #eee;background:#faf9f7}.vz-nhead h3{margin:0;font-size:17px;font-family:'Playfair Display',serif}.vz-nhead button{border:0;background:#f4f1ec;color:#111;width:34px;height:34px;border-radius:50%;font:inherit;font-size:18px;display:grid;place-items:center;flex-shrink:0}.vz-list{overflow:auto;overscroll-behavior:contain;padding:12px;flex:1}.vz-item{border:1px solid #e8e3dc;border-radius:13px;padding:12px;margin-bottom:10px;cursor:pointer}.vz-item.unread{border-color:#111;background:#faf9f7}.vz-item strong{display:block}.vz-item small{display:block;color:#777;margin-top:5px}.vz-empty{padding:30px 12px;text-align:center;color:#777}.vz-shade{position:fixed;inset:0;background:#0006;z-index:175;display:none}.vz-shade.open{display:block}@media(min-width:700px){.vz-shade.open{display:none}}`;document.head.appendChild(s)}
    const btn=document.createElement('button');btn.id='vz-notify';btn.className='icon-btn';btn.type='button';btn.setAttribute('aria-label','الإشعارات');btn.innerHTML='🔔';(document.querySelector('.top-actions')||document.querySelector('.header-actions'))?.prepend(btn); if(!btn.isConnected)return;
    const badge=document.createElement('span');badge.className='vz-badge';badge.hidden=true;btn.appendChild(badge);
    const shade=document.createElement('div');shade.className='vz-shade';document.body.appendChild(shade); const panel=document.createElement('aside');panel.className='vz-panel';panel.innerHTML=`<div class="vz-nhead"><h3>الإشعارات</h3><button type="button" data-nclose>×</button></div><div class="vz-list" data-nlist></div>`;document.body.appendChild(panel);
    ...
    const load=async()=>{const {data}=await client.from('notifications').select('id,title,body,read_at,created_at,order_id').eq('admin_user_id',session.user.id).order('created_at',{ascending:false}).limit(100);render(data||[])}; const open=()=>{panel.classList.add('open');shade.classList.add('open');document.body.style.overflow='hidden';load()}; const close=()=>{panel.classList.remove('open');shade.classList.remove('open');document.body.style.overflow=''}; btn.onclick=open; shade.onclick=close;panel.querySelector('[data-nclose]').onclick=close;
    ...
    document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')ensurePush(client,session)});
