(() => {
  ...
  const start = async () => {
    ...
    const renderAddresses = async () => {
      ...
      box.innerHTML = addresses?.length
        ? addresses
            .map(
              (a) =>
                `<div class="auth-address"><div><strong>${esc(a.label)}</strong><span>${esc(a.address)}</span></div><button type="button" class="auth-address-remove" data-address-remove="${a.id}">حذف</button></div>`,
            )
            .join('')
        : '<p class="auth-note">ما عندكش عناوين محفوظة بعد.</p>';
      box.querySelectorAll('[data-address-remove]').forEach(
        (btn) =>
          (btn.onclick = async () => {
            await client
              .from('customer_addresses')
              .delete()
              .eq('id', btn.dataset.addressRemove)
              .eq('user_id', currentUserId);
            renderAddresses();
          }),
      );
    ...
    const renderOrderItemsHtml = (items) =>
      (items || [])
        .map(
          (it) =>
            `<div class="auth-order-item"><span>${it.product_name || it.name} × ${it.quantity || it.qty}${it.color ? ' · ' + it.color : ''}${it.size ? ' · ' + it.size : ''}</span><span>${Number((it.unit_price || it.price || 0) * (it.quantity || it.qty || 1)).toLocaleString('ar-LY')} د.ل</span></div>`,
        )
        .join('');
    ...
    const reorderItems = (items) => {
      ...
      (items || []).forEach((it) => {
        ...
        const existing = window.cart?.find?.(
          (c) => c.id === p.id && c.color === (it.color || p.colors?.[0]) && c.size === size,
        );
    ...
    const showAccount = async (session) => {
      if (!session) {
        ...
        tabsBar.classList.remove('auth-hidden');
        trackSection.classList.remove('auth-hidden');
        account.classList.add('auth-hidden');
        currentUserId = null;
        return;
      }
      form.classList.add('auth-hidden');
      tabsBar.classList.add('auth-hidden');
      trackSection.classList.add('auth-hidden');
      account.classList.remove('auth-hidden');
      currentUserId = session.user.id;
      const { data: profile } = await client
        .from('customer_profiles')
        .select('full_name,phone,preferred_size,marketing_opt_in')
        .eq('id', session.user.id)
        .maybeSingle();
      modal.querySelector('[data-auth-welcome]').textContent =
        `أهلاً ${profile?.full_name || session.user.email || 'بك'} 👋`;
      ...
      try {
        const { data: remoteWishlist } = await client
          .from('wishlist_items')
          .select('product_id')
        ...
        if (toUpload.length)
          await client
            .from('wishlist_items')
            .insert(toUpload.map((product_id) => ({ user_id: session.user.id, product_id })));
      ...
      const loadItems = async (orderId) => {
        ...
        const { data } = await client
          .from('order_items')
          .select('product_name,product_code,color,size,quantity,unit_price')
          .eq('order_id', orderId);
        itemsCache[orderId] = data || [];
        return itemsCache[orderId];
      };
      ordersBox.querySelectorAll('[data-order-details]').forEach(
        (btn) =>
          (btn.onclick = async () => {
            const box = modal.querySelector(
              `[data-order-detail-box="${btn.dataset.orderDetails}"]`,
            );
            const isHidden = box.classList.contains('auth-hidden');
            if (isHidden) {
              const items = await loadItems(btn.dataset.orderDetails);
              box.innerHTML =
                renderOrderItemsHtml(items) || '<p class="auth-note">ما فيش تفاصيل لهذا الطلب.</p>';
            }
            box.classList.toggle('auth-hidden', !isHidden);
          }),
      );
      ordersBox.querySelectorAll('[data-order-reorder]').forEach(
        (btn) =>
          (btn.onclick = async () => {
    ...
    modal.querySelector('[data-address-add]').onclick = async () => {
      ...
      if (!address) {
        alert('اكتب تفاصيل العنوان أولاً.');
        return;
      }
    ...
    modal.querySelector('[data-forgot-password]').onclick = async () => {
      ...
      if (!email) {
        setStatus('اكتب إيميلك أولاً بالخانة فوق، وبعدين دوس نسيت كلمة المرور.');
        return;
      }
      ...
      setStatus(
        error ? error.message || 'تعذر الإرسال' : 'أرسلنا رابط إعادة تعيين كلمة المرور لإيميلك ✓',
      );
    };
    ...
    trackBtn.onclick = async () => {
      ...
      if (!phone || !orderNumber) {
        trackResult.textContent = 'لازم تكتب رقم الهاتف ورقم الطلب.';
        return;
      }
      ...
      try {
        const r = await fetch(
          `/api/track-order?phone=${encodeURIComponent(phone)}&order_number=${encodeURIComponent(orderNumber)}`,
        );
        const data = await r.json();
        if (!r.ok || !data.ok) {
          trackResult.textContent = data.error || 'تعذر العثور على الطلب.';
          return;
        }
        const o = data.order;
        const itemsHtml = (o.items || [])
          .map(
            (it) =>
              `<div class="auth-order"><strong>${it.product_name}</strong><span>${it.product_code} · الكمية: ${it.quantity}</span></div>`,
          )
          .join('');
        ...
        trackResult.textContent = 'تعذر الاتصال بالسيرفر.';
      } finally {
    ...
    tabs.forEach(
      (tab) =>
        (tab.onclick = () => {
          ...
          nameField.classList.toggle('auth-hidden', mode !== 'signup');
    ...
    client.auth.onAuthStateChange(async (_event, session) => {
      if (modal.classList.contains('open')) await showAccount(session);
    });
    ...
    if (whatsapp) {
      ...
      let originTop = 0;
      const getSideLeft = (x) =>
        x < window.innerWidth / 2 ? 0 : Math.max(0, window.innerWidth - whatsapp.offsetWidth);
      const getTop = (top) =>
        Math.min(Math.max(top, 0), Math.max(0, window.innerHeight - whatsapp.offsetHeight));
      const restore = () => {
        try {
      ...
      const save = () => {
        try {
          ...
          localStorage.setItem(
            key,
            JSON.stringify({
              side: Number.isFinite(right) ? 'right' : 'left',
              top: parseFloat(whatsapp.style.top),
            }),
          );
        } catch (_) {}
      };
